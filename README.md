...
1. **模型家族**:回复所用模型属于 Flash 系列(含 `deepseek-flash` 与仍在路由的旧名 `deepseek-v4-flash`、`deepseek-v4-flash-vision-exp`)就用 Flash 价表;其它模型(含无法识别的)按 Pro 价表计。官方自 2026-09-14 12:00(北京时间)起把 `deepseek-v4-pro` 的请求全部路由到 V4.1-Flash,插件从同一时刻起也改按 Flash 价表计它;
2. **时间时段**:按北京时间的官方调价节点分段——**2026-08-17 00:00 之前**按旧价表(不分峰谷);**2026-08-17 00:00 起**启用峰谷定价,高峰为 9:00–12:00、14:00–18:00,当时不分工作日与周末;**2026-09-10 12:00 起** Flash 系列降价,高峰同时收窄为**工作日**的 9:00–12:00、14:00–18:00,周末与其余时段一律算空闲时段;
...
内置价表(元 / 百万 token,生效时间均为北京时间):

| 模型 | 生效时间 | 时段 | 缓存命中 | 未命中输入 | 输出 |
| --- | --- | --- | ---: | ---: | ---: |
| DeepSeek-V4.1-Flash | 2026-09-10 12:00 起 | 空闲(含周末) | 0.02 | 1.0 | 4.0 |
| DeepSeek-V4.1-Flash | 2026-09-10 12:00 起 | 高峰(工作日) | 0.04 | 2.0 | 8.0 |
| DeepSeek-V4-Flash | 2026-08-17 00:00 起 | 空闲 | 0.05 | 1.5 | 4.5 |
| DeepSeek-V4-Flash | 2026-08-17 00:00 起 | 高峰(每天) | 0.10 | 3.0 | 9.0 |
| DeepSeek-V4-Flash | 2026-08-17 之前 | 不分峰谷 | 0.02 | 1.0 | 2.0 |
| DeepSeek-V4-Pro | 2026-08-17 之前 | 不分峰谷 | 0.025 | 3.0 | 6.0 |
| DeepSeek-V4-Pro | 2026-08-17 00:00 起 | 空闲 | 0.15 | 4.5 | 13.5 |
| DeepSeek-V4-Pro | 2026-08-17 00:00 起 | 高峰(工作日) | 0.30 | 9.0 | 27.0 |

> 2026-09-14 12:00(北京时间)起,`deepseek-v4-pro` 的请求由官方路由到 V4.1-Flash,按上表 Flash 价计费。
...
- 展开区底部的"费率说明"显示**最后一条回复的模型**与**当前时段**的单价(不是该会话的加权平均),换了模型或跨过峰谷/调价时点就会跟着变。
...
- 价格表内置在代码里,官方调价后需要更新插件版本(官方没有价格查询接口);调价时在 `src/pricing.ts` 的价表里**追加一段**并重新发版即可,历史会话仍按当时的费率逐条计费;
- 文案目前是中文。
...
cd deepseek-harness
git checkout ddefc45fbc7f8e46dd73185e68295696d1297887   # tag dsh-v0.1.6-alpha.2,CI 钉住的同一个 ref
...
当前对齐的 Session API(0.1.6-alpha.2):Session 列表快照 `SessionListState` 里**已经没有 `current` 字段**,卡片按主视图持有的那一行取当前会话(`retainedBy.mainView > 0`),规则集中在 `src/client/session.ts`(同时对 0.1.6 之前仍带 `current` 的快照保留兜底),改动它请同步 `tests/session.test.ts`。
