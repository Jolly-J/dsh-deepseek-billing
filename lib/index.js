...
*/
async function fetchBalance(apiKey, apiKeyEnv, baseURL) {
	...
	try {
		...
		if (!response.ok) return {
			keyMissing: false,
			balance: null,
			balanceError: `余额接口请求失败: ${`${response.status} ${response.statusText}`.trim()}${text.length > 0 ? ` (${text.slice(0, MAX_ERROR_BODY)})` : ""}`
		};
		if (text.length === 0) return {
			keyMissing: false,
			balance: null,
			balanceError: "余额接口返回空响应"
		};
		try {
...
function eraAt(timeMs) {
	let current = LEGACY_ERA;
	for (const era of ERAS) if (timeMs >= era.from) current = era;
	return current;
}
...
function tierAt(timeMs, weekdayOnly = true) {
...
function modelName(modelId, timeMs) {
