 ...
 */
export function selectActiveSessionId(list: SessionListLike | null | undefined): string | null {
  if (list === null || list === undefined) return null
  const retained = Object.values(list.byId ?? {})
    .find(row => (row?.retainedBy?.mainView ?? 0) > 0)
  if (retained !== undefined && retained !== null) return String(retained.id)
  const legacy = list.current
  return legacy === undefined || legacy === null ? null : String(legacy)
}
